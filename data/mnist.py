...
def create_semisupervised(x, y, n_labeled):
    ...
    for i in range(n_classes):
        ...
        x_lab = np.ndarray((x[i].shape[0], n_labels_per_class))
        y_lab = np.ndarray((y[i].shape[0], n_labels_per_class))
        x_unlab = np.ndarray((x[i].shape[0], x[i].shape[1] - n_labels_per_class))
        y_unlab = np.ndarray((y[i].shape[0], y[i].shape[1] - n_labels_per_class))

        for j, item in enumerate(idx):
            if (j < n_labels_per_class):
                x_lab[:,j] = x[i][:, item]
                y_lab[:,j] = y[i][:,item]
            else:
               x_unlab[:,j - n_labels_per_class] = x[i][:,item]
               y_unlab[:,j - n_labels_per_class] = y[i][:,item]
        x_labeled[i] = x_lab
        y_labeled[i] = y_lab
        x_unlabeled[i] = x_unlab
        y_unlabeled[i] = y_unlab
