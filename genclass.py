...
class GenerativeClassifier( object ):
	...
	def _generate_zxy( self, x, y, reuse = False, sample=True ):
	...
	def _objective( self ):
		...
		def L(x_recon, x, y, z):
			...
			if self.distributions['p_y'] == 'uniform':
				...
				log_prior_y = - tf.nn.softmax_cross_entropy_with_logits( logits=y_prior, labels=y )#maybe switch labels and logits
		...
		with self.session as sess:
			...
			for epoch in range(epochs):
				...
				for x_l_mu, x_l_lsgms, y, x_u_mu, x_u_lsgms in utils.feed_numpy_semisupervised(
					...
					_data_labelled[:,:int(2*self.dim_x)], _data_labelled[:,int(2*self.dim_x):],_data_unlabelled ):
	...
	def predict_labels( self, x_test, y_test ):
		...
		x_test_mu = x_test[:,:int(self.dim_x)]
		x_test_lsgms = x_test[:,int(self.dim_x):int(2*self.dim_x)]
	...
	def encode( self, x, y, sample = False ):
		if sample:
			return self.session.run( [self.z_lab, self.z_lab_mu, self.z_lab_lsgms], feed_dict = { self.x_lab: x, self.y_lab: y } )
		else:
			return self.session.run( [self.z_lab_mu, self.z_lab_lsgms], feed_dict = { self.x_lab: x, self.y_lab: y } )

	def decode( self, z, y):
		return self.session.run( [self.x_recon_lab_mu, self.x_recon_lab_lsgms], feed_dict = { self.z_lab: z, self.y_lab: y } )
